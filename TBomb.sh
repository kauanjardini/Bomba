...
banner() {
    ...
    if ! [ -x "$(command -v toilet)" ]; then
        echo -e "\e[4;34m This Bomber Was Created By \e[1;32mKauan \e[0m"
    else
    ...
    echo -e "\e[4;32m   YouTube: https://youtube.com/channel/UCf4CRMZSQ1wysKBIJm3P8aw \e[0m"
    echo " "
...
install_deps(){
    ...
    if [ -n "$INSTALL" ];then
        ...
        echo "As dependencias não estão instaladas."
...
init_environ
if [ -f .update ];then
    echo " os requerimentos não estão instalados...."
else
    echo 'Instalando Requirementos....'
    echo .
    echo .
    install_deps
    echo This Script Was Made By SpeedX > .update
    echo 'Requirementos Instalados....'
    pause
fi
while :
do
    ...
    echo " "
    echo "Press 1 para  Comecar Bomba de SMS  "
    echo "Press 2 para Comecar Bomba de CALL  "
    echo "Press 3 para  Comecar Bomba de MAIL  (Not Yet Available)"
    echo "Press 4 To  Update (Works On Linux And Linux Emulators) "
    echo "Press 5 Para Sair "
    read ch
    clear
    if [ $ch -eq 1 ];then
